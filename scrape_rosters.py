...
def load_profile_cache():
    ...
    Self-heals against the legacy doubled-URL bug: if >50% of cached entries
    are 'failed', we assume the cache is poisoned and start fresh.
    """
    ...
    try:
        ...
        # Poisoned-cache detection: too many failures => discard & re-fetch
        if cache:
            failed = sum(
                1 for v in cache.values()
                if v.get('profile_scraped') == 'failed'
            )
            failed_share = failed / len(cache)
            if failed_share > 0.5:
                print(f"  Profile cache has {failed_share:.0%} failures "
    ...
    async with async_playwright() as p:
        ...
        for i, (team, season) in enumerate(tasks):
            ...
            if skip_existing and ckpt.exists():
                ...
                # Also detect the legacy doubled-URL bug — if every row's
                # profile_url contains 'comhttps' the checkpoint is poisoned
                # and must be re-scraped from scratch.
                try:
                    head = pd.read_csv(ckpt, nrows=1)
                    has_transfer_cols = 'transfer_origin_team' in head.columns
                    sample = pd.read_csv(ckpt, usecols=['profile_url'])
                    bad_url_share = (
                        sample['profile_url'].astype(str)
                ...
                if poisoned:
                    print(f"[{i+1}/{len(tasks)}] REDO {team} {season} "
                          f"(checkpoint has malformed profile URLs — bug fixed)")
