...
PROFILE_SELECTOR_TIMEOUT_MS = 15000   # bumped from 8s — slow profiles need it
...
TRANSFER_FIELDS = [
    ...
    #                       (true negative — player never transferred or transfer is in future)
...
def _parse_one_section(section, is_juco_title):
    ...
    rating, year (int|None), overall_rank, position_rank, position, stars.
    """
    out = {
        'kind': '', 'rating': '', 'year': None,
        'overall_rank': '', 'position_rank': '', 'position': '', 'stars': '',
    }
    ...
    for li in section.select('li'):
        ...
        if 'OVR' in label or 'OVERALL' in label:
            ...
            # National rank — only relevant for prospect sections; ignore for transfer
            continue
        elif 'State=' in href:
            # State rank — not what we want
            continue
        elif not out['position_rank']:
            # First non-OVR/non-state rank => position rank
...
def parse_player_profile(html):
    """
    Pull ALL transfer events + the origin/destination teams from a profile.
    ...
    Returns:
      {
        ...
        'transfer_events': [          # list, ordered as found on page
          {'kind':'Transfer','rating':'88','year':2024,
           'overall_rank':'123','position_rank':'5','position':'WR',
           'stars':'4'}, ...
        ],
        'section_titles': [...],      # for diagnostics
      }

    Why a list of events? A 6th-year transfer-twice player has TWO Transfer
    sections. For a roster scrape we need to pick the event whose (YYYY)
    matches the roster season — caller does that match, not us. Per playbook
    rule #1: NEVER `break` after the first Transfer section.
    """
    soup = BeautifulSoup(html, 'lxml')
    result = {
        'origin_team': '',
    ...
    for section in soup.select('section.rankings-section'):
        ...
        if 'Transfer' in title:
            ...
            # Recruiting/JUCO data — not what we're after but kept for
            # potential future use; we don't surface these to the roster row.
            pass
...
def pick_transfer_event(profile, season):
    ...
    Rule: prefer the most recent event whose year is <= season. If no such
    event exists (player's only transfer is FUTURE relative to the roster
    year, like a 2024 freshman with a 2026 transfer entry), return None
    so we DON'T pollute that roster row with future data.
    """
    events = profile.get('transfer_events') or []
    if not events:
        return None
    # Filter to events at-or-before this season (ignore future transfers)
    eligible = [e for e in events if e.get('year') and e['year'] <= season]
    if not eligible:
        return None
    # Most recent year wins; tie-break by original page order (later in list)
    eligible.sort(key=lambda e: (e.get('year') or 0,))
    return eligible[-1]
            ...
            try:
                ...
                if attempt < MAX_PROFILE_RETRIES - 1:
                    ...
                    return {
                        ...
                        'transfer_events': [], 'section_titles': [],
                        'fetch_status': 'failed',
                    }
        return {
            'origin_team': '', 'destination_team': '',
...
def apply_profile_to_row(row, profile, season):
    """Pick the right transfer event for this season and write transfer
    fields into the row. Sets profile_scraped to one of:
    ...
    if event is None:
        ...
        # Still record origin/destination teams (they're profile-level, not event-level)
        # so analysts can see "this player is on Alabama, came from Auburn" even
        # without a transfer event recorded.
